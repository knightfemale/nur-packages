/*
  功能:
    ...
    不匹配则 builtins.warn 警告, 不阻断求值
  输入参数:
    prefix: 用于校验错误提示的路径前缀 (递归时自动拼接)
    schema: 嵌套 attrs, 叶子为 lib.types.* 值
    attrs: 待校验的 attrs
  返回值:
    原始 attrs (不变), 校验结果通过 builtins.warn 输出
  示例:
      ...
      checkOne =
        ...
        if attrs ? ${key} then
          ...
          if builtins.isAttrs schemaVal && !(schemaVal ? check) then
            ...
            builtins.warn "${fullKey} expected type ${schemaVal.name}, but ${toString val} is ${builtins.typeOf val}" true
          else
            true
        else
          true;
